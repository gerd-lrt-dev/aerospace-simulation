...
StateVector`}</FlowBlock>
        ...
        <section id="control" className="flowSection">
          ...
          <p>
            ...
            conversion from those commands into main-engine and RCS actuator states. Main-engine automation,
            translational RCS control, and rotational RCS control are resolved as separate command channels.
          </p>
          ...
          <div className="layerFlow">
            <div className="archLayer archLayerFrontend">
              ...
              <FlowBlock>{`Manual RCS axes ─→ inputmapper ─────┐
                                      │
...
Main engine + RCS target commands
        ↓
Thrust / RCSControlAllocator`}</FlowBlock>
            </div>
          </div>
          ...
          <p>
            ...
            the interface boundary, and arbitration, automatic control, simulation orchestration, and propulsion are
            backend responsibilities.
          </p>

          <h3>Rotational-control authority</h3>

          <p>
            Manual rotational input is used while neither automatic attitude mode is active. When
            <code>killRotation</code> or <code>stabilize</code> is active, <code>InputArbiter</code> replaces only the
            rotational channel with the command generated by <code>AttitudeController</code>. Translational RCS input
            remains manual, and main-engine automation remains independently controlled by the autopilot flag.
          </p>

          <FlowBlock>{`Neither attitude mode active:
...
AttitudeController::stabilize()
  ...
  └─ hysteresis state
        ↓
autoCmd.rotation ──────────────────────────→ active rotation`}</FlowBlock>

          <h3>Backend step order</h3>

          <p>
            The automatic-control command is evaluated from the current spacecraft state before actuation targets are
            applied and before the physical state advances. The resulting telemetry snapshot is generated only after
            the completed spacecraft update.
          </p>

          <FlowBlock>{`Frontend command transfer
        ↓
...
InputArbiter mode flags ─────┘`}</FlowBlock>
            </div>
...
TelemetryDTO`}</FlowBlock>
   ...
   └──→ stateUpdated(...)
       ...
       cockpit / visualization`}</FlowBlock>
          ...
          <h3>Attitude-control mode telemetry</h3>

          <p>
            The active rotational automation mode is read from <code>InputArbiter</code> after the spacecraft step and
            added to the same <code>simData</code> snapshot. The interface layer maps these values into the dedicated
            <code>TelemetryDTO::Automation</code> group.
          </p>

          <FlowBlock>{`InputArbiter
...
TelemetryDTO::Automation
  ...
  └─ stabilizeActive
        ↓
Cockpit history / XML export`}</FlowBlock>
...
SimulationWorker
   ...
   └──→ telemetryHistory_
          ...
          <FlowBlock>{`<automation>
  <killRotationActive>true|false</killRotationActive>
  <stabilizeActive>true|false</stabilizeActive>
</automation>`}</FlowBlock>

          <p>
            This allows controller activation, settling behavior, and RCS response to be correlated precisely during
            telemetry-based verification without inferring the control mode indirectly from thruster activity.
          </p>
        ...
        <section id="ownership" className="flowSection">
          ...
          <div className="flowGrid">
            ...
            <div className="flowCard">
              ...
              <p>
                <strong>InputArbiter</strong> owns the current manual/automatic authority flags for main-engine,
                Kill Rotation, and Stabilize command selection.
              </p>
            </div>

            <div className="flowCard">
              <h3>Attitude-control state</h3>
              <p>
                <strong>AttitudeController</strong> owns the captured Stabilize target quaternion and its internal
                hysteresis/correction state. It does not own or directly modify spacecraft physical state.
              </p>
            </div>
